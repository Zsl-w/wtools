...
pub struct FilePreviewResult {
    pub preview_type: String, // "text", "image", "binary"
    pub content: Option<String>, // 文本内容（前 N 行）
...
// 获取文件预览（文本文件返回前 N 行，图片返回 base64，其他返回元数据）
#[tauri::command]
...
fn extract_text_from_stream(content: &[u8]) -> String {
    // 从 PDF 内容流中提取文本
    // PDF 文本操作符: BT ... ET 包含文本块，Tj/TJ 输出文本
